...
public class AzureBatchSource extends AbstractFileSource<AzureBatchSource.AzureBatchConfig> {
  ...
  protected Map<String, String> getFileSystemProperties(BatchSourceContext context) {
    ...
    if (path != null && (path.startsWith(AzureBatchConfig.SCHEME_WASB) || path.startsWith(AzureBatchConfig.SCHEME_WASBS))) {
    ...
    } else if (path != null && (path.startsWith(AzureBatchConfig.SCHEME_ABFS) || path.startsWith(AzureBatchConfig.SCHEME_ABFSS))) {
  ...
  protected void recordLineage(LineageRecorder lineageRecorder, List<String> outputFields) {
    lineageRecorder.recordRead("Read", "Read from Azure Storage.", outputFields);
  }
  ...
  public static class AzureBatchConfig extends AbstractFileSourceConfig {
    ...
    private static final String SCHEME_WASB = "wasb://";
    private static final String SCHEME_WASBS = "wasbs://";
    private static final String ACCOUNT_SUFFIX_WASB = ".blob.core.windows.net";
    private static final String SCHEME_ABFS = "abfs://";
    private static final String SCHEME_ABFSS = "abfss://";
    private static final String ACCOUNT_SUFFIX_ABFS = ".dfs.core.windows.net";
    ...
    public void validate(FailureCollector collector) {
      ...
      boolean isWasb = !containsMacro(NAME_PATH) &&
              path != null && (path.startsWith(SCHEME_WASB) || path.startsWith(SCHEME_WASBS));
      boolean isAbfs = !containsMacro(NAME_PATH) &&
              path != null && (path.startsWith(SCHEME_ABFS) || path.startsWith(SCHEME_ABFSS));

      if (!containsMacro(NAME_PATH) && !isWasb && !isAbfs) {
        collector.addFailure(String.format(
          "Path must start with %s, %s, %s, or %s.", AzureBatchConfig.SCHEME_WASB, AzureBatchConfig.SCHEME_WASBS,
                        AzureBatchConfig.SCHEME_ABFS, AzureBatchConfig.SCHEME_ABFSS), null)
          .withConfigProperty(NAME_PATH);
      }

      if (!containsMacro(NAME_ACCOUNT) && account != null) {
        if (isWasb && !account.endsWith(AzureBatchConfig.ACCOUNT_SUFFIX_WASB)) {
          collector.addFailure(
            String.format("Account must end with '%s' for %s paths.", AzureBatchConfig.ACCOUNT_SUFFIX_WASB,
                    AzureBatchConfig.SCHEME_WASB), null)
            .withConfigProperty(NAME_ACCOUNT);
        }
        if (isAbfs && !account.endsWith(ACCOUNT_SUFFIX_ABFS)) {
          collector.addFailure(
                  String.format("Account must end with '%s' for %s paths.", AzureBatchConfig.ACCOUNT_SUFFIX_ABFS,
                          AzureBatchConfig.SCHEME_ABFS), null)
            .withConfigProperty(NAME_ACCOUNT);
        }
      }
